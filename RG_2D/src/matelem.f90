...
!Local variables
    ...
    real(wp)       tAk(nn,nn),tAl(nn,nn),tAkl(nn,nn),rmkl1(nn,nn),Vkl1,Hkl1,Ckl
    real(wp)       inv_Akk(nn,nn),inv_All(nn,nn),inv_tAkl(nn,nn), inv_tAkltAl(nn,nn)
    real(wp)       inv_invtAkinvtAl(nn,nn)
    ...
    real(wp)       lambda_k1(nn+1),lambda_k2(nn+1),lambda_l1(nn+1),lambda_l2(nn+1),Zeta(nn+1,nn+1)
...
!Scalar-Gaussian overlap divided by 4*pi^(3/2).  The factor 1/4
!belongs to the normalized D-state angular prefactors.  Keeping the
!angular polynomial separate avoids divisions by m or tau3, either of
!which can vanish for a finite off-diagonal density matrix element.
    if (AreCorrFuncNeeded.or.ArePartDensNeeded) then
      temp1=Glob_PiRaised3n2/(FOUR*Glob_Pi*Glob_SqrtPi*det_tAkl*sqrt(det_tAkl))
    endif
...
!Evaluation of correlation functions
    if (AreCorrFuncNeeded) then
      ...
      do i=1,n
        do j=i,n
          ...
          temp3=-1/TrAJ(j,i)
          temp4=-temp3
          term1=tau3*eta2(j,i)+tau33*eta22(j,i) &
                +tau333*eta223(j,i)+tau334*eta224(j,i)
          term2=eta(j,i)
          !The constrained D-state angular polynomial is
          ! c0 + c2*(xi_rho^2+4*xi_z^2)
          !    + c4*(xi_rho^2-2*xi_z^2)^2.
          !Precompute its three coefficients outside the grid loop.
          temp5=m-term1*temp4+term2*temp4*temp4
          temp6=ONETHIRD*(term1-TWO*term2*temp4)*temp4*temp4
          temp7=ONETHIRD*term2*temp4*temp4*temp4*temp4
          do k=1,NumCFGridPoints
            temp8=CFGrid(2,k)*CFGrid(2,k)          !this is \xi_z^2
            temp9=CFGrid(1,k)*CFGrid(1,k)          !this is \xi_rho^2
            temp10=temp8+temp9                     !this is \xi^2
            temp11=temp9-TWO*temp8
            temp12=temp9+FOUR*temp8
            temp13=temp5+temp6*temp12+temp7*temp11*temp11
            CFkl(p,k)=temp2*temp13*exp(temp10*temp3)
          enddo
        enddo
      enddo
    endif
    ...
    if (ArePartDensNeeded) then
      do i=1,n+1
        ...
        enddo
        term1=tau3*temp5*temp6+tau33*temp3*temp4 &
              +tau333*temp5*temp4+tau334*temp3*temp6
        term2=TWO*temp3*temp4*temp5*temp6
        temp3=ONE/temp2
        temp4=m-term1*temp3+term2*temp3*temp3
        temp5=ONETHIRD*(term1-TWO*term2*temp3)*temp3*temp3
        temp6=ONETHIRD*term2*temp3*temp3*temp3*temp3
        temp7=temp1/(sqrt(temp2)*temp2)
        do k=1,NumDensGridPoints
          temp8=DensGrid(2,k)*DensGrid(2,k)          !this is \xi_z^2
          temp9=DensGrid(1,k)*DensGrid(1,k)          !this is \xi_rho^2
          temp10=temp8+temp9                         !this is \xi^2
          temp11=temp9-TWO*temp8
          temp12=temp9+FOUR*temp8
          temp13=temp4+temp5*temp12+temp6*temp11*temp11
          Denskl(i,k)=temp7*temp13*exp(-temp10*temp3)
        enddo
      enddo
    endif
...
!For computational efficiency, Zeta=eta/4 and the lambda vectors are half
!the corresponding quantities in the Fourier-transform formulas.
    ...
    if (AreMCorrFuncNeeded) then
      ...
      do i=1,n
        do j=i,n
          ...
          temp66b=temp7*temp8*temp9*temp10
          temp3=temp1/(temp2*sqrt(temp2))
          temp13=temp55b/(24*temp2*temp2)
          temp14=temp66b/(96*temp2**4)
          temp7=-ONE/(FOUR*temp2)
          do k=1,NumCFGridPoints
            temp4=CFGrid(1,k)*CFGrid(1,k)
            temp5=CFGrid(2,k)*CFGrid(2,k)
            temp6=temp4+temp5
            temp11=temp4+FOUR*temp5
            temp12=temp4-TWO*temp5
            CFkl(p,k)=temp3*(temp44b-temp13*temp11+temp14*temp12*temp12)*exp(temp6*temp7)
          enddo
        enddo
      enddo
    endif
    ...
    if (AreMomDensNeeded) then
      ...
      do i=1,n+1
        ...
        temp66b=lambda_k1(i)*lambda_l1(i)*lambda_k2(i)*lambda_l2(i)
        temp3=temp1/(temp2*sqrt(temp2))
        temp13=temp55b/(24*temp2*temp2)
        temp14=temp66b/(96*temp2**4)
        temp7=-ONE/(FOUR*temp2)
        do k=1,NumDensGridPoints
          ...
          temp11=temp4+FOUR*temp5
          temp12=temp4-TWO*temp5
          Denskl(i,k)=temp3*(temp44b-temp13*temp11+temp14*temp12*temp12)*exp(temp6*temp7)
        enddo
      enddo
    endif
