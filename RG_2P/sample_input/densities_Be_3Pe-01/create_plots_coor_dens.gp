...
#   <name>_axial_dist.png  ->  2*pi*xi_rho * rho(xi_rho, xi_z)
#
...
do for [c=3:ncol] {
    ...
    # The finite-mass nuclear distribution is orders of magnitude narrower
    # than the electronic one.  Both windows preserve the grid aspect ratio.
    if (name eq "rho1") {
        direct_rho = 0.0006
        direct_z = 0.0006
        axial_rho = 0.00065
        ...
        set xtics 0.0002
        set ytics 0.0002
        # Contours on this strongly stretched half-plane grid develop false
        # open/closed segments near the narrow maximum.  Interpolate the
        # color surface, but do not overlay misleading contour curves.
    ...
    set cbrange [0:STATS_max]
    if (name eq "rho1") {
        set cntrparam levels incremental STATS_max/20.0, STATS_max/10.0, 15.0*STATS_max/20.0
    } else {
        set cntrparam levels incremental STATS_max/20.0, STATS_max/10.0, 19.0*STATS_max/20.0
    }
