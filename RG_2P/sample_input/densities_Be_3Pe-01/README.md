...
To reproduce the results, from the repository root, build the double-precision, five-particle executable:

```bash
./build.bash machine=linux-generic toolchain=systemdefault config=release code=RG_2P nparticles=5 precision=8 linalg=netlib
```

then generate the grid and run the ECGPACK calculations:

```bash
cd RG_2P/sample_input/densities_Be_3Pe-01
python3 create_cyl_grid.py
mkdir -p output_cyl
mpirun -np 1 ../../../bin/systemdefault/release/RG_2P_N5_P8_netlib
mv -f cf.dat dens.dat mcf.dat mdens.dat expvals.txt spinData.txt swapfile.dat output_cyl/
```

The paths in this command sequence are relative to the working directory `RG_2P/sample_input/densities_Be_3Pe-01`, and the example starts one MPI process. To change the number of MPI processes, edit:
...
On a four-core Intel Core i5-1135G7 system under WSL, the supplied 16,836-point grid and 100-function basis take approximately 3 minutes with four MPI processes. This is a separate four-process benchmark; the command above uses one process. Build time is not included. Runtime varies with the compiler, linear-algebra library, processor, system load, basis size, grid size, and MPI process count.

The executable must be built for the `PARTICLES 5` value in `inout.txt`.
