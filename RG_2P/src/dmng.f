...
C  PORT optimization routines by David M. Gay (Bell Laboratories)
C=====================================================================
C
C  This file contains a modified copy of the unconstrained
C  minimization routines of the PORT Mathematical Subroutine Library,
C  taken from the publicly available subset distributed by Netlib:
C
C      https://www.netlib.org/port/
C
C  Per the Netlib PORT readme, the NL2SOL variants and the MNF/MNG/MNH
C  minimizers (of which the DMNG/DRMNG routines below are the double
C  precision MNG) belong to the publicly available portion of PORT.
C  The separate limited-use agreement mentioned in that readme applies
C  to the complete PORT library, not to this subset.
C
C  The algorithms are documented in:
C
C    (1) Dennis, J.E., Jr., Gay, D.M., and Welsch, R.E. (1981),
C        "An adaptive nonlinear least-squares algorithm",
C        ACM Trans. Math. Software, Vol. 7, No. 3, pp. 348-368.
C
C    (2) Gay, D.M. (1983), "Algorithm 611: Subroutines for
C        unconstrained minimization using a model/trust-region
C        approach", ACM Trans. Math. Software, Vol. 9, No. 4,
C        pp. 503-524.
C
C---------------------------------------------------------------------
C  Copyright and permission notice of the original software
C---------------------------------------------------------------------
C
C  All software in the directory is Copyright Lucent Technologies.
C  Permission to use, copy, modify, and distribute this software for
C  any purpose without fee is hereby granted, provided that this
C  entire notice is included in all copies of any software which is
C  or includes a copy or modification of this software and in all
C  copies of the supporting documentation for such software.
C
C  THIS SOFTWARE IS BEING PROVIDED "AS IS", WITHOUT ANY EXPRESS OR
C  IMPLIED WARRANTY.  IN PARTICULAR, NEITHER THE AUTHORS NOR LUCENT
C  TECHNOLOGIES MAKE ANY REPRESENTATION OR WARRANTY OF ANY KIND
C  CONCERNING THE MERCHANTABILITY OF THIS SOFTWARE OR ITS FITNESS
C  FOR ANY PARTICULAR PURPOSE.
C
C---------------------------------------------------------------------
C  Modifications made for ECGPACK
C---------------------------------------------------------------------
C
C  The original source has been modified only in its handling of the
C  real kind: DOUBLE PRECISION declarations were replaced by REAL(wp)
C  and each routine now obtains the kind parameter wp from module
C  wp_def, so that the minimizer can be built in double (fp64),
C  extended (fp80), or quadruple (fp128) precision.  The numerical
C  algorithms themselves are unchanged.  Bugs in this copy should be
C  reported to the ECGPACK authors, not to Netlib or the original
C  author.
C
C  See THIRD-PARTY-NOTICES.md in the root of the ECGPACK repository.
C=====================================================================
C All subroutines use module wp_def where the kind parameter wp (for real 
C numbers) is defined. No other information is used from module wp_def. 
...
C
      LOGICAL GOODX
      INTEGER I, NFC
      REAL(wp) EMAX, EMAXS, GTS, RFAC1, XMAX
      REAL(wp) HALF, ONE, ONEP2, TWO, ZERO
C
...
C
C/6
C     DATA HALF/0.5E0_wp/, ONE/1.E0_wp/, ONEP2/1.2E0_wp/, TWO/2.E0_wp/,
C    1     ZERO/0.E0_wp/
C/7
      PARAMETER (HALF=0.5E0_wp, ONE=1.E0_wp, ONEP2=1.2E0_wp, TWO=2.0_wp,
     1           ZERO=0.E0_wp)
C/
C
C/6
...
C
      INTEGER LV, N
      REAL(wp) DIG(N), NWTSTP(N), STEP(N), V(LV)
C
C  ***  PURPOSE  ***
C
...
C
      INTEGER I
      REAL(wp) CFACT, CNORM, CTRNWT, GHINVG, FEMNSQ, GNORM,
     1                 NWTNRM, RELAX, RLAMBD, T, T1, T2
      REAL(wp) HALF, ONE, TWO, ZERO
C
C  ***  V SUBSCRIPTS  ***
C
...
C
C/6
C     DATA HALF/0.5E0_wp/, ONE/1.E0_wp/, TWO/2.E0_wp/, ZERO/0.E0_wp/
C/7
      PARAMETER (HALF=0.5E0_wp, ONE=1.E0_wp, TWO=2.E0_wp, ZERO=0.E0_wp)
C/
C
C/6
...
C
C/6
C     DATA ONE/1.E0_wp/, SQTETA/0.E0_wp/, ZERO/0.E0_wp/
C/7
      PARAMETER (ONE=1.E0_wp, ZERO=0.E0_wp)
      DATA SQTETA/0.E0_wp/
C/
C
      DD7TPR = ZERO
...
C/
      REAL(wp) NRELDF, OLDF, PRELDF, RELDF, ZERO
C
...
C
 ...
 100  FORMAT(I6,I5,E27.20,2E9.2,E8.1,A3,A4,2E8.1,E9.2)
 110  FORMAT(I6,I5,E27.20,2E10.2,3E9.1,E10.2)
C
...
C     *** FUNCTION EVALUATION LIMIT IS 1...
 ...
 410  FORMAT(/6H     0,I5,E27.20)
 420  FORMAT(/6H     0,I5,E27.20)
      GO TO 999
C
...
C
      ...
      REAL(wp) A, B, BJ, ETA, GJ, LJ, LIJ, LJJ, NU, S, THETA,
     1                 WJ, ZJ
      REAL(wp) ONE, ZERO
C
C  ***  DATA INITIALIZATIONS  ***
C
C/6
C     DATA ONE/1.E0_wp/, ZERO/0.E0_wp/
C/7
      PARAMETER (ONE=1.E0_wp, ZERO=0.E0_wp)
C/
C
...
C/
      REAL(wp) BIG, MACHEP, TINY, VK, VM(34), VX(34), ZERO
C
...
C/
C
      DATA BIG/0.E0_wp/, MACHEP/-1.E0_wp/, TINY/1.E0_wp/, ZERO/0.E0_wp/
C/6S
...
C/
C
      DATA VM(1)/1.E-3_wp/,VM(2)/-0.99E0_wp/,VM(3)/1.E-3_wp/,
     1 VM(4)/1.E-2_wp/,VM(5)/1.2E0_wp/,VM(6)/1.E-2_wp/,VM(7)/1.2E0_wp/,
     2 VM(8)/0.E0_wp/,VM(9)/0.E0_wp/,VM(10)/1.E-3_wp/,VM(11)/-1.E0_wp/,
     3 VM(13)/0.E0_wp/,VM(15)/0.E0_wp/,VM(16)/0.E0_wp/,VM(19)/0.E0_wp/,
     4 VM(20)/-10.E0_wp/,VM(21)/0.E0_wp/,VM(22)/0.E0_wp/,
     5 VM(23)/0.E0_wp/,VM(27)/1.01E0_wp/,VM(28)/1.E10_wp/,
     6 VM(30)/0.E0_wp/,VM(31)/0.E0_wp/,VM(32)/0.E0_wp/,VM(34)/0.E0_wp/
      DATA VX(1)/0.9E0_wp/,VX(2)/-1.E-3_wp/,VX(3)/1.E1_wp/,
     1 VX(4)/0.8E0_wp/,VX(5)/1.E2_wp/,VX(6)/0.8E0_wp/,VX(7)/1.E2_wp/,
     2 VX(8)/0.5E0_wp/,VX(9)/0.5E0_wp/,VX(10)/1.E0_wp/,VX(11)/1.E0_wp/,
     3 VX(14)/0.1E0_wp/,VX(15)/1.E0_wp/,VX(16)/1.E0_wp/,VX(19)/1.E0_wp/,
     4 VX(23)/1.E0_wp/,VX(24)/1.E0_wp/,VX(25)/1.E0_wp/,VX(26)/1.E0_wp/,
     5 VX(27)/1.E10_wp/,VX(29)/1.E0_wp/,VX(31)/1.E0_wp/,VX(32)/1.E0_wp/,
     6 VX(33)/1.E0_wp/,VX(34)/1.E0_wp/
C
C/6S
...
C
      REAL(wp) D1MACH, ZERO
      EXTERNAL D1MACH
      DATA BIG/0.E0_wp/, ETA/0.E0_wp/, MACHEP/0.E0_wp/, ZERO/0.E0_wp/
...
C
 10   DR7MDC = ETA
      GO TO 999
C
 20   DR7MDC = SQRT(256.E0_wp*ETA)/16.E0_wp
      GO TO 999
C
 30   DR7MDC = MACHEP
      GO TO 999
C
 40   DR7MDC = SQRT(MACHEP)
      GO TO 999
C
 50   DR7MDC = SQRT(BIG/256.E0_wp)*16.E0_wp
      GO TO 999
C
 60   DR7MDC = BIG
C
...
C  ***  LAST CARD OF DR7MDC FOLLOWS  ***
      END
      REAL(wp) FUNCTION DRLDST(P, D, X, X0)
      USE wp_def		  
C
...
C
      INTEGER P
      REAL(wp) D(P), X(P), X0(P)
C
      INTEGER I
      REAL(wp) EMAX, T, XMAX, ZERO
C/6
C     DATA ZERO/0.E0_wp/
C/7
      PARAMETER (ZERO=0.E0_wp)
...
C     ***  CONSTANTS  ***
C
      REAL(wp) HALF, NEGONE, ONE, ONEP2, ZERO
C
...
C
C/6
C     DATA HALF/0.5E0_wp/, NEGONE/-1.E0_wp/, ONE/1.E0_wp/, ONEP2/1.2E0_wp/,
C    1     ZERO/0.E0_wp/
C/7
      PARAMETER (HALF=0.5E0_wp, NEGONE=-1.E0_wp)
      PARAMETER (ONE=1.E0_wp, ONEP2=1.2E0_wp,
     1           ZERO=0.E0_wp)
C/
C
C+++++++++++++++++++++++++++++++  BODY  ++++++++++++++++++++++++++++++++
...
C
      INTEGER I, J
      REAL(wp) ONE, R, SCALE, SQTETA, T, XI, ZERO
C/+
      REAL(wp) SQRT
C/
      REAL(wp) DR7MDC
...
C DR7MDC... RETURNS MACHINE-DEPENDENT CONSTANTS
C
      REAL(wp) MACHEP, MEPCRT, ONE, SQTEPS, THREE
C
C  ***  SUBSCRIPTS FOR V  ***
C
...
C
C/6
C     DATA ONE/1.E0_wp/, THREE/3.E0_wp/
C/7
      PARAMETER (ONE=1.E0_wp, THREE=3.E0_wp)
C/
C
C  ***  V SUBSCRIPT VALUES  ***
C
...
C
      MACHEP = DR7MDC(3)
      V(AFCTOL) = 1.E-20_wp
      IF (MACHEP .GT. 1.E-10_wp) V(AFCTOL) = MACHEP**2
      V(DECFAC) = 0.5E0_wp
      SQTEPS = DR7MDC(4)
      V(DFAC) = 0.6E0_wp
      V(DTINIT) = 1.E-6_wp
      ...
      V(TUNER5) = 0.75E0_wp
      V(XCTOL) = SQTEPS
      V(XFTOL) = 1.E2_wp * MACHEP
C
      IF (ALG .GE. 2) GO TO 10
C
C  ***  REGRESSION  VALUES
C
      V(COSMIN) = MAX(1.E-6_wp, 1.E2_wp * MACHEP)
      V(DINIT) = 0.E0_wp
      V(DELTA0) = SQTEPS
      V(DLTFDC) = MEPCRT
      V(DLTFDJ) = SQTEPS
...
C
      INTEGER I
      REAL(wp) CS, CY, EPS, EPSRT, ONE, SHS, YS, THETA
C
C  ***  DATA INITIALIZATIONS  ***
C
C/6
C     DATA EPS/0.1E0_wp/, ONE/1.E0_wp/
C/7
      PARAMETER (EPS=0.1E0_wp, ONE=1.E0_wp)
C/
C
C+++++++++++++++++++++++++++++++  BODY  ++++++++++++++++++++++++++++++++
C
