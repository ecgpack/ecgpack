...
!functions (elements of the Cholesky matrix, L_k, and matrix B_k)
...
!Array Glob_H is used to store the Hamiltonian matrix
  complex(wp),allocatable,dimension(:,:),save  :: Glob_H

!Array Glob_S is used to store the overlap matrix
  complex(wp),allocatable,dimension(:,:),save  :: Glob_S
...
!Index j ranges from 1 to Glob_nfa
  complex(wp),allocatable,dimension(:,:,:),save  :: Glob_D

!Array Glob_c is used to store the eigenvector
  complex(wp),allocatable,dimension(:),save :: Glob_c

!Array Glob_invD is used only when Glob_GSEPSolutionMethod='I'.
!It contains inverse elements of matrix D when
!H-Glob_ApproxEnergy*S is factorized in the Cholesky form, L*D*LH.
  complex(wp),allocatable,dimension(:),save :: Glob_invD
...
!fewer iterations may be needed.
  complex(wp),allocatable,dimension(:),save :: Glob_LastEigvector
...
!the Hamiltonian and the overlap matrix elements
  complex(wp),allocatable,dimension(:),save ::  &
...
!=============================================================
  ...
  complex(wp),allocatable,dimension(:)  :: Glob_WorkForZHEGVX
  real(wp),allocatable,dimension(:)  :: Glob_RWorkForZHEGVX
  integer,allocatable,dimension(:)  :: Glob_IWorkForZHEGVX
  real(wp),allocatable,dimension(:)  :: Glob_WkGR
  complex(wp),allocatable,dimension(:)  :: Glob_WorkForGHEPIIS
!=============================================================
