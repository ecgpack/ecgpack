...
| `FIXED_INDEX_1` | optional | This optional line can only be used in the case of `RG_2D` or `RG_2P` basis types. The keyword is followed by the fixed value of the first integer index that should be used in calculations. If `VECTOR_COUPLING_SCHEME` is also present, only one of `FIXED_INDEX_1` and `FIXED_INDEX_2` may be supplied. <br> *Example* : `FIXED_INDEX_1 2` |
| `FIXED_INDEX_2` | optional | This optional line can only be used in the case of `RG_2D` or `RG_2P` basis types. The keyword is followed by the fixed value of the second integer index that should be used in calculations. If `VECTOR_COUPLING_SCHEME` is also present, only one of `FIXED_INDEX_1` and `FIXED_INDEX_2` may be supplied. <br> *Example* : `FIXED_INDEX_2 3` |
...
For the `RG_2D` basis type the two additional integers are the indices of the $x$- and $y$-premultipliers, each ranging from 1 to the number of pseudoparticles $n$. The two indices of a given function determine the vector coupling scheme it realizes: two different indices correspond to two pseudoparticles in $p$-states coupled to a $D$-state, whereas two equal indices correspond to a single pseudoparticle in a $d$-state. By default a basis may contain functions of both kinds; the optional header keyword `VECTOR_COUPLING_SCHEME` restricts the generation and optimization of the indices to just one of the two schemes.
