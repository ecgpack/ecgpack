#!/usr/bin/env gnuplot
# ---------------------------------------------------------------------------
# create_plots_mom_cf.gp
#
# Reads mcf.dat:
#   col 1 = eta_rho (x-axis)
#   col 2 = eta_z   (y-axis)
#   col 3..N = momentum correlation functions f_i (names taken from the header)
#
# For every data column it produces two PNG contour/density plots:
#   <name>.png             ->  f(eta_rho, eta_z)
#   <name>_axial_dist.png  ->  2*pi*eta_rho*f(eta_rho, eta_z)
#
...
set title "f({/Symbol h}_{/Symbol r}, {/Symbol h}_z)"   # ... and a one-line title, so the
...
do for [c=3:ncol] {
    ...
    subscript = substr(name, 2, strlen(name))
    glabel = "f_{" . subscript . "}"

    # The nucleus-electron momentum correlation is much broader than the
    # electron-electron one, so use independently scaled windows.
    if (name eq "f1") {
        direct_rho = 9.0
        direct_z = 9.0
        axial_rho = 13.0
    ...
    # ---- plot 2: integration kernel 2*pi*eta_rho*f -----------------------
    set autoscale x
    set autoscale y
