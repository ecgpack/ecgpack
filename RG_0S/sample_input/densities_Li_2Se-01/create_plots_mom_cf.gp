#!/usr/bin/env gnuplot
#
# Plot the momentum correlation functions f from mcf.dat.
...
datafile = output_dir . "/mcf.dat"
cleanfile = "mcf_plot.tmp"
...
do for [i=1:words(names)] {
    ...
    set ylabel sprintf("f_{%s}({/Symbol h})",subscript)
    set title sprintf("Momentum correlation function f_{%s}({/Symbol h})",subscript)
    plot cleanfile using 1:col with lines lw 4 title sprintf("f_{%s}({/Symbol h})",subscript)
    ...
    set ylabel sprintf("4{/Symbol p}{/Symbol h}^2 f_{%s}({/Symbol h})",subscript)
    set title sprintf("Momentum radial distribution 4{/Symbol p}{/Symbol h}^2 f_{%s}({/Symbol h})",subscript)
    plot cleanfile using 1:(4*pi*$1**2*column(col)) with lines lw 4 title sprintf("4{/Symbol p}{/Symbol h}^2 f_{%s}({/Symbol h})",subscript)
}
